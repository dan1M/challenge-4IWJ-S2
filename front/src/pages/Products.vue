...
const findFirstInStockVariant = (variants) => {
            ...
            <div class="mt-10 grid gap-10 md:grid-cols-2 lg:gap-10 xl:grid-cols-3 ">
                <div :class="{ 'group cursor-pointer': true }" v-for="product in products" :key="product.id">
                    <router-link :to="{ name: 'detailProduct', params: { id: product._id } }">
                        <div
                            :class="{ 'overflow-hidden rounded-md bg-gray-100 transition-all hover:scale-105 dark:bg-gray-800': true }">
                    ...
                    <div class="flex items-center justify-between">
                                ...
                                <div class="mt-2 flex items-center space-x-2 text-gray-500 dark:text-gray-400">
                                    <div>
                                        ...
                                        <h2 :class="[
                                            ...
                                            <span class="bg-gradient-to-r from-pink-200 to-pink-100 bg-[length:0px_10px] bg-left-bottom
                                                                            bg-no-repeat
                        ...
                        <div>
                            <Button
                                @click="addToCart(findFirstInStockVariant(product.variants).color.id, findFirstInStockVariant(product.variants).size.id, product._id)"
                                :disabled="isOutOfStock(product.variants)">
                                {{ isOutOfStock(product.variants) ? 'Rupture de stock' : 'Ajouter au panier' }}
                            </Button>
                        </div>
                    </div>
                </div>
